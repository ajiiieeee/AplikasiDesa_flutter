...
class KadesHomeScreen extends StatefulWidget {
  ...
  @override
  State<KadesHomeScreen> createState() => _KadesHomeScreenState();
}

class _KadesHomeScreenState extends State<KadesHomeScreen> {
  String _namaUser = 'Kepala Desa';
  String _fotoProfil = '';
  int _menungguTTE = 0;
  int _selesai = 0;
  int _ditolak = 0;
  ...
  List<PengajuanModel> _suratMenunggu = [];
  ...
  Future<void> _loadProfile() async {
    ...
    final nama = prefs.getString('nama_lengkap') ?? prefs.getString('nama') ?? 'Kepala Desa';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (body['status'] == 'success') {
          ...
          if (mounted) {
            setState(() {
              _menungguTTE = (data['menunggu_persetujuan'] as num?)?.toInt() ?? 0;
              _selesai = (data['selesai'] as num?)?.toInt() ?? 0;
              _ditolak = (data['ditolak'] as num?)?.toInt() ?? 0;
              _totalPengaduan = (data['total_pengaduan'] as num?)?.toInt() ?? 0;
              _isLoading = false;
            });
          }
        }
  ...
  Future<void> _fetchSuratMenunggu() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final pending = list.where((item) => item.status == 'Disetujui Sekretaris Desa' || item.status.isEmpty).toList();
        if (mounted) {
          setState(() {
            _suratMenunggu = pending.take(5).toList();
          });
        }
      }
    } catch (e) {
            ...
            children: [
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        GestureDetector(
                          onTap: () => Navigator.push(context,
                              MaterialPageRoute(builder: (_) => InfoProfile())),
                          child: Container(
                            decoration: BoxDecoration(
                              shape: BoxShape.circle,
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    // STATISTIK 4 KARTU
                    Text('Statistik Pengesahan Surat',
                        style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.bold, color: const Color(0xFF263238))),
                              ...
                              Row(
                                children: [
                                  Expanded(child: _buildStatCard(
                                    title: 'Menunggu TTE Kades',
                                    count: '$_menungguTTE',
                                    subtitle: 'Perlu Pengesahan Final',
                    ...
                    // SECTION SURAT MENUNGGU PERSETUJUAN
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text('Surat Menunggu Persetujuan',
                            style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.bold, color: const Color(0xFF263238))),
                        Text('5 Terbaru',
                    ...
                    _suratMenunggu.isEmpty
                        ? Container(
                            ...
                            child: Center(
                              child: Text('Tidak ada surat yang menunggu pengesahan',
                                  style: GoogleFonts.poppins(fontSize: 13, color: Colors.grey[600])),
                            ),
                          )
                        : Column(
                            children: _suratMenunggu.map((item) {
                              return Container(
                                ...
                                child: ListTile(
                                  ...
                                  leading: Container(
                                    ...
                                    child: const Icon(Icons.draw_rounded, color: Color(0xFFE65100), size: 22),
                                  ),
                                  title: Text(item.namaPemohon,
                                      style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.bold, color: const Color(0xFF263238))),
                                  subtitle: Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      Text(item.namaSurat, style: GoogleFonts.poppins(fontSize: 12, color: const Color(0xFF546E7A))),
                                      Text(item.tanggalDiajukan, style: GoogleFonts.poppins(fontSize: 10, color: Colors.grey[500])),
                                    ],
                                  ),
                                  trailing: Container(
                                    ...
                                    child: Text('Menunggu TTE',
                                        style: GoogleFonts.poppins(fontSize: 9, fontWeight: FontWeight.bold, color: const Color(0xFFE65100))),
                                  ),
                                ),
                              );
                            }).toList(),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text(title, style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 12, color: const Color(0xFF263238))),
